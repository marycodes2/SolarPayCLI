class Region < ActiveRecord::Base
	...
	def self.return_user_region(state)
		region = (1..10).to_a
		region_names = region.map do |num|
			str = Region.find(num).name
			arr = str.split("-")
			array = arr.map {|elem| elem.split("+") }.flatten
			array.delete_if {|elem| elem == "USA"}
		end
		state = state.to_s
		state = state.upcase
		# region_1 = ['AK', 'HI']
		...
		elsif region_names[0].include?(state)
			Region.find(1)
		elsif region_names[1].include?(state)
			Region.find(2)
		elsif region_names[2].include?(state)
			Region.find(3)
		elsif region_names[3].include?(state)
			Region.find(4)
		elsif region_names[4].include?(state)
			Region.find(5)
		elsif region_names[5].include?(state)
			Region.find(6)
		elsif region_names[6].include?(state)
			Region.find(7)
		elsif region_names[7].include?(state)
			Region.find(8)
		elsif region_names[8].include?(state)
			Region.find(9)
		elsif region_names[9].include?(state)
			Region.find(10)
		else
...
def find_solar_break_even(cost_of_solar, q1_consumption, q2_consumption, q3_consumption, q4_consumption)
  ...
  quarters.each do |quarter|
    if quarter.to_s.split('.')[1].to_i == 25
      total_cost += (find_slope_of_region_line(quarter) / 100) * q2_consumption
      break_even_quarter = quarter
      break if total_cost > cost_of_solar
    elsif quarter.to_s.split('.')[1].to_i == 5
      total_cost += (find_slope_of_region_line(quarter) / 100) * q3_consumption
      break_even_quarter = quarter
      break if total_cost > cost_of_solar
    elsif quarter.to_s.split('.')[1].to_i == 75
      total_cost += (find_slope_of_region_line(quarter) / 100) * q4_consumption
      break_even_quarter = quarter
      break if total_cost > cost_of_solar
    else
      total_cost += (find_slope_of_region_line(quarter) / 100) * q1_consumption
      break_even_quarter = quarter
      break if total_cost > cost_of_solar
    end
  end
